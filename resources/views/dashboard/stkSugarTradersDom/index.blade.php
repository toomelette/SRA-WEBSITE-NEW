...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            active = '';
            stk_sugar_traders_dom_tbl = $("#block_farm_table").DataTable({
                'dom' : 'lBfrtip',
                "processing": true,
                "serverSide": true,
                "ajax" : '{{ route("dashboard.stkSugarTradersDom.index") }}',
                "columns": [
                    { "data": "year" },
                    { "data": "date"},
                    { "data": "file_title" },
            ...
            $('#block_farm_table_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    stk_sugar_traders_dom_tbl.search(this.value).draw();
                }
            });
        });
        ...
        $("#roadMapForm").submit(function (e) {
            ...
            loading_btn(form);
            $.ajax({
                url: "{{route('dashboard.stkSugarTradersDom.store')}}",
                type: 'POST',
                ...
                success: function (res) {
                    ...
                    Swal.fire({
                        title: 'Success!',
                        text: 'Your New Sugar Trader successfully added to Sugar Traders Domestic!',
                        icon: 'success',
