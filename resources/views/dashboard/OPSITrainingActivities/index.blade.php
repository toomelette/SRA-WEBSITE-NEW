                        ...
                        <form id="roadMapForm" method="POST" autocomplete="off" enctype="multipart/form-data">
                            ...
                            <div class="box-body">
                                ...
                                <div class="col-md-12">
                                    ...
                                    {!! __form::textbox(
                                      '12', 'description', 'text', 'Description *', '', old('title'), $errors->has('title'), $errors->first('title'), 'required'
                                    ) !!}
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            active = '';
            opsi_training_activities_tbl = $("#block_farm_table").DataTable({
                'dom' : 'lBfrtip',
                "processing": true,
                "serverSide": true,
                "ajax" : '{{ route("dashboard.OPSITrainingActivities.index") }}',
                "columns": [
                    { "data": "year" },
                    { "data": "date"},
                    { "data": "file_title" },
            ...
            $('#block_farm_table_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    opsi_training_activities_tbl.search(this.value).draw();
                }
            });
        });
        ...
        $("#roadMapForm").submit(function (e) {
            ...
            loading_btn(form);
            $.ajax({
                url: "{{route('dashboard.OPSITrainingActivities.store')}}",
                type: 'POST',
                ...
                success: function (res) {
                    ...
                    Swal.fire({
                        title: 'Success!',
                        text: 'New Activity successfully added to OPSI Training Activities!',
                        icon: 'success',
