...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            active = '';
            block_farm_established_visayas_tbl = $("#block_farm_table").DataTable({
                'dom' : 'lBfrtip',
                "processing": true,
                "serverSide": true,
                "ajax" : '{{ route("dashboard.blockFarmEstablishedVisayas.index") }}',
                "columns": [
                    { "data": "year" },
                    { "data": "date"},
                    { "data": "file_title" },
            ...
            $('#block_farm_table_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    block_farm_established_visayas_tbl.search(this.value).draw();
                }
            });
        });
        ...
        $("#roadMapForm").submit(function (e) {
            ...
            loading_btn(form);
            $.ajax({
                url: "{{route('dashboard.blockFarmEstablishedVisayas.store')}}",
                type: 'POST',
