...
<p>
@php
    $Template = \App\Models\BlockFarmHYVNurseriesVis::query()->get()->sortByDesc('id');
                            ...
                            <ul>
                                @foreach ($Template as $template)
                                    @if($year->name == $template->year)
                                        <li class="text-justify"><a class="btn" style="color: #ffb600" target="_blank" href="/view_file/block_farm_established_hyv_nurseries_visayas/{!!$template->slug!!}" >{!!$template->title!!},</a>{!!$template->description!!}</li>
                                    @endif
                                @endforeach
