...
<p>
@php
    $Template = \App\Models\StationPampangaExtserv::query()->get()->sortByDesc('id');
                            ...
                            <ul>
                                @foreach ($Template as $template)
                                    @if($year->name == $template->year)
                                        <li class="text-justify"><a class="btn" style="color: #ffb600" target="_blank" href="/view_file/station_pampanga_extserv/{!!$template->slug!!}" >{!!$template->file_title!!},</a>{!!$template->title!!}</a></li>
                                    @endif
                                @endforeach
